...
if gen_invoice:
    if not file_invoice:
        ...
        try:
            ...
            if df is None:
                st.error("无法读取文件，请检查：1. 文件无损坏 2. 文件为标准.xlsx格式")
                st.error("\n".join(error_messages))
                st.stop()
